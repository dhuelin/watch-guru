...
| Sign-in and session renewal | Written, never run, and the part that cannot be verified by reading alone: it needs a real Apple Developer account and a device. The equivalent Android logic *is* tested on a plain JVM, which is some evidence about the design but none about this code. |
